...
from ofrom_outils.common import (
    CORP, SYMS, iter_top_tiers, iter_segs, get_top_tiers,
    iter_file, get_spk, fix_lext
)
from ofrom_outils.pr.private_paths import sub_corpus
from ofrom_outils.formats import fromOFROM
...
def wd_simple(tr: Transcription, spk: str = "") -> int:
    ...
    for seg in iter_segs(tr, spk=spk):
        wd += 1
    return wd
def wd_xml(tr: Transcription, spk: str = "") -> int:
...
def wd_full(tr: Transcription, spk: str = "") -> int:
    ...
    for seg in iter_segs(tr, spk=spk):
        ...
        wd += 1
    return wd
def dur_simple(tr: Transcription, spk: str = "") -> float:
...
D_FUN = {
    ...
    # Lecture #
    #---------#
def fill_stlist(st: StList, fi: str):
    st.wd += st.fi[fi].wd
    st.dur += st.fi[fi].dur
    for spk, tpl in st.fi[fi].spk.items():
        if spk not in st.spk:
            st.spk[spk] = [el for el in tpl]; continue
        st.spk[spk][0] = tpl[0]
        st.spk[spk][1] = tpl[1]
