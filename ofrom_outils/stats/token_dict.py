...
from ofrom_outils.common import (
    ROOT, TAGS, SYMS, iter_file, iter_core, iter_segs
)
...
class TokenDict(AbsTokenDict):
    ...
    def _get_anno(self, seg: Segment, l_atier: list[Tier]) -> list[str]:
        ...
        return l_res
    def _getRowDict(self, sh: Worksheet) -> dict[str, int]:
        """Lignes par token."""
        d_sh = {}; a = 2
        for row in sh.iter_rows(2):
            d_sh[row[1].value] = a; a+=1
        return d_sh
    def ofrom_dict_c(self, word: str) -> None|str:
    ...
    def generate(self) -> OFRDict:
        ...
        for corp, fi, trans, tok in self._iter_toks():
            ...
            if (not seg.content) or re.search(SYMS, seg.content):
                continue                        # ignore symbols
            c = seg.content[0]
            c = "tronc" if seg.content.endswith("-") else c
            c = d_match[c] if c in d_match else c
            c = "autre" if not c in self.d else c # get 'c' key
            l_anno = _getAnno(seg,[ps_tier,le_tier,ph_tier]) # get anno'
            if not seg.content in self.d[c]:      # new word
                self.d[c][seg.content] = {
                    "pos":[l_anno[0]],
                    "lemma":[l_anno[1]],
                    "phones":l_anno[2],
                    "nb":1,
                ...
                self.d[c][seg.content]['nb'] += 1
                if not l_anno[0] in self.d[c][seg.content]['pos']:
                    self.d[c][seg.content]["pos"].append(l_anno[0])
                if not l_anno[1] in self.d[c][seg.content]['lemma']:
                    self.d[c][seg.content]["lemma"].append(l_anno[1])
                if not trans.name in self.d[c][seg.content]['files']:
                    self.d[c][seg.content]['files'].append(fi)
        return self.d
    ...
    def save(self, path: Path = "") -> None:
        ...
        path = self.f if not path else path
        wb = xl.Workbook(); sh = wb.active; sh.title = "autre"
        sh.append(HEAD)
        for c in L_SH:
            if c == "autre":
                sh = wb["autre"]
            elif c not in wb.sheetnames:
                sh = wb.create_sheet(); sh.title = c
                sh.append(HEAD)
            else:
                sh = wb[c]
            d_sh = self._getRowDict(sh)
            d_c = d_wd[c]
            l_c = list(d_c.keys())
            l_c.sort()
            for k in l_c:
