...
def _writeU(ttab, a, n, seg, s, e):
    ...
    return a+1, n
def _writeW(ttab, i, n, typ, s, e, w, p, l):
    """Writes a word."""
...
def _writeBody(f, ntrans, d_timetable, id):
    ...
    for seg in ntrans.iterTime(l_par):
        ...
        iu, txt = _writeU(ttab, iu, txt, seg, s, e) # utterance
        d_cti = d_par[tier]
        ...
        for i in range(len(l_ws)):                  # words (mwu)
            ...
            iw, txt = _writeW(ttab, iw, txt, typ, s, e, w, p, l)
        f.write(txt)
        txt = ""
    if iu > 0:
        txt = txt+"{ttab}</u>\n"
    f.write(txt+tab+"\t</div>\n"+tab+"</body>\n\t</text>\n</TEI>")   
def saveTXM(path,trans,encoding,ext):
...
def _saveList(path,trans,encoding,ext):
    ...
    for tr in trans:
        saveTXM(path,tr,encoding,ext)

    # Main function
def toTXM(path,trans,**args):
    ...
    if f:
        ...
        raise KeyError("First argument must be of type 'Transcription/"+
                       "/Corpus/list'.")
d_load = {Transcription:saveTXM, Corpus:_saveList,
          list:_saveList}