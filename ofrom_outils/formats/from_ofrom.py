...
def _check_files(path, ch_ext=".xml"):
    """Returns a list of '.xml' files."""

    def _check_file(fpath, afile, l_afiles):
        fi, ext = os.path.splitext(afile)
        if not ext.lower() == ch_ext:
            return
        l_afiles.append((fpath, fi))
    ...
    if os.path.isdir(path):  # 'path' is a directory
        for file in os.listdir(path):
            ...
            _check_file(p, file, l_files)
        ch_dir = 1
    elif os.path.isfile(path):  # 'path' is a file
        file = os.path.basename(path)
        _check_file(path, file, l_files)
    else:
        return [], -1
    return l_files, ch_dir


def _read_meta(trans, elem):
...
def _read_anno(trans, elem):
    """<table_tok_min/mwu>, fills annotation tiers."""

    def deal_with_lemma(suba, spka, tmina, tmaxa):
        nname = spka + "[lemma]"
        ntier = trans.getName(nname)
        if spka and not ntier:
            ptier = trans.getName(spka)
            i = ptier.index() + 1 if ptier else -1
            ntier = trans.create(i, nname)
        nseg = ntier.getTime(tmina)
        if not nseg:
            ntier.create(-1, "", tmina, tmaxa, suba.get('lemma', ""))
        else:
            nseg.content = suba.get('lemma', "")

    def deal_with_anno(suba, minwua, spka, tmina, tmaxa, typ):
        pmin, pmwu = suba.get('pos_min'), suba.get('pos_mwu')
        if typ == "pos":
            txt = pmin if pmin else pmwu
        else:
            txt = suba.get('text')
        nname = spka + "[" + typ + "_" + minwua + "]"
        ntier = trans.getName(nname)
        if spka and not ntier:
            ptier = trans.getName(spka)
            i = ptier.index() + 1 if ptier else -1
            ntier = trans.create(i, nname)
        ntier.create(-1, "", tmina, tmaxa, html.unescape(txt))
    ...
    for sub in elem.iter('tok' + minwu):
        ...
        if 'lemma' in sub.keys():
            deal_with_lemma(sub, spk, tmin, tmax)  # lemma tier
        deal_with_anno(sub, minwu, spk, tmin, tmax, "pos")  # PoS tier
        deal_with_anno(sub, minwu, spk, tmin, tmax, "tok")  # Tok tier


d_OFROMoper = {'metadata': _read_meta,
               'statistics': _read_stats,
               'table_soundsegment': _read_trans,
               'table_tok_mwu': _read_anno,
               'table_tok_min': _read_anno}


def load_ofrom(path, name=""):
...
def from_ofrom(path, **_args):
    ...
    # Get files
    l_files, ch_dir = _check_files(path)
    if ch_dir == 1:  # list of files
        l_trans = []
        for tup in l_files:
            l_trans.append(load_ofrom(*tup))
        return l_trans
    elif ch_dir == 0 and l_files:  # single file
        return load_ofrom(*l_files[0])
    return None